<?php 
set_time_limit(700); //extender tiempo en que se puede cargar
$opts = array(
    'http'=>array(
      'method'=>"GET",
...
while($boolean){
  while ($columna = mysqli_fetch_array( $resultado3))
  {
      ...
      // echo $IDC;
      // echo "\n";
      // echo "".$decoded_json [ 'data' ][$a] [ 'detRutDoc']."-".$decoded_json [ 'data' ][$a] [ 'detDvDoc']."\n";
      // echo var_export($IDC == "".$decoded_json [ 'data' ][$a] [ 'detRutDoc']."-".$decoded_json [ 'data' ][$a] [ 'detDvDoc']."",true);
      if($IDC == "".$decoded_json [ 'data' ][$a] [ 'detRutDoc']."-".$decoded_json [ 'data' ][$a] [ 'detDvDoc'].""){
      $boolean=FALSE;
      $pk_emp = $columna['PK'] ;
      }
  }
if($boolean){
    ...
    $consulta3 = "SELECT * FROM `empresa` ORDER BY `PK` DESC";
    $resultado3 = mysqli_query( $conexion, $consulta3 ) or die ( "Algo ha ido mal en la consulta a la base de datos 2do aviso");
...
$boolean= TRUE; //no existe
$pk_tip =-1;
$consulta3 = "SELECT * FROM `tipomovimiento`";
...
while($boolean){
  ...
  {
      $IDC = $columna['Nombre'] ;
      if($IDC == $decoded_json [ 'data' ][$a] [ 'descTipoTransaccion']){
      $boolean=FALSE;
      $pk_tip = $columna['PK'] ;
      }
  }
